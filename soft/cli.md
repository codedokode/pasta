...
Командная строка - это способ взаимодействия с комьютером, когда ты печатаешь команды, а он их исполняет. Она может использоваться для запуска программ (например на языке PHP) и для настройки некоторых программ.
...
* Ubuntu: Запустить приложение Терминал  или нажать Alt + Ctrl + F1  (вернуться назад в графический режим - Ctrl + F7)
* Другие дистрибутивы Linux: если ты линуксоид, ты и так знаешь

## Клавишы, работающие во всех операционных системах
...
В командной строке результат выполнения команд часто зависит от того, в каком каталоге и на каком диске ты находишься. Также, когда ты указваешь в команде относительное имя файла (без полного пути и имени диска), оно ищется в текущем каталоге.
...
В linux PATH — это папки /bin, /usr/bin, /usr/local/bin, /sbin, /usr/sbin, именно в них в Линуксе хранятся почти все программы. В текущем каталоге програмы не ищутся, если это не указать явно, набрав `./command`.
...
\#2 — поток ошибок. Сюда правильно написанная программа пишет сообщения об ошибках, о ходе выполнения задачи. Для них специально преудсмотрен отдельный поток (чтобы при сохранении результатов работы в файл в него не записывались сообщения о ходе работы), но по умолчанию так же как и #1 он подклюен к консоли.
