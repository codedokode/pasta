...
Если ты не очень хорошо знаком с исключениями, советую перечитать [урок по ним](php/exceptions.md).
...
Ты можешь добавлять к таблице и отдельным колонкам комментарии: [/db/comments.md](./db/comments.md). Эти комментарии сохраняются в базе и выводятся в программах для работы с ней. Добавляй комментарии там, где назначение колонки не очень очевидно, они помогут другим людям разобраться.
...
В этой задаче необходимо хранить данные в базе. Потому, убедись что ты имеешь хотя бы базовые знания языка SQL. Из PHP с базой данных можно работать через одно из двух расширений: PDO ([статья на Хабре](http://habrahabr.ru/post/137664/), [мануал](http://php.net/manual/ru/book.pdo.php)) и mysqli ([хабр](http://habrahabr.ru/post/141127/), [мануал](http://php.net/manual/ru/book.mysqli.php)). Расширение mysql (без i на конце) устарело. Если ты видишь учебник, где используются устаревшие функции вроде `mysql_query()` — выбрасывай это старье.

У MySQLi и PDO есть сильные и слабые стороны, если интересно, то можно погуглить или поискать их в мануале: http://php.net/manual/ru/mysqli.overview.php . При использовании любой из библиотек, необходимо включить режим выброса исключений при ошибках. Если это не сделать, то либо информация об ошибках не будет нигде выводиться, либо придется после вызова каждой функции ставить `if` с проверкой результата. К сожалению, по умолчанию в обоих библиотеках режим выброса исключений выключен.
...
Также кодировку можно задать SQL-запросом `SET NAMES utf8mb4`.

Разумеется, при создании новой базы данных (SQL-запрос `CREATE DATABASE`) ей также рекомендуется задать кодировку `utf8mb4`, чтобы создаваемые в ней таблицы поддерживали хранение любых символов Юникода.
...
Через плейсхолдеры можно подставлять только числа и строки. Если надо подставить в SQL-код имя таблицы или поля, то придется вставлять переменную прямо в запрос, перед этим проверив ее по списку разрешенных значений.
...
Чтобы получить id только что вставленной в базу записи, есть специальная надежная функция [lastInsertId](http://php.net/manual/ru/pdo.lastinsertid.php) в PDO и поле [insert_id](http://php.net/manual/ru/mysqli.insert-id.php) в MySQLi. Не изобретай велосипедов.
...
Учебники описывают работу через командную строку. Кроме утилит командной строки, работать с репозиторием можно через утилиты с графическим интерфейсом, также поддержка CVS встроена в популярные IDE вроде Eclipse, Netbeans или PhpStorm. Однако, изучать CVS проще всего именно в командной строке (так как там не требуется разбираться в сложном интерфейсе с множеством кнопок и опций), а, уже после, если хочется, можно попробовать использовать GUI программы.
