...
Еще удобнее собирать такие ссылки с помощью стандартной функции [http_build_query](http://php.net/manual/ru/function.http-build-query.php):
...
Можно сделать удобный класс для рассчета числа страниц и формирования ссылок на переход на нужную страницу. В конструктор мы передаем общее число записей, число записей на странице, шаблон для ссылки, после чего может генерировать ссылки для перехода: 

```php
$pager = new Pager($totalPages, $recordsPerPage, 'index.php?page=%d');
