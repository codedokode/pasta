...
Если строка записана в двойных кавычках, то в нее можно вставить произвольные байты, зная их код в 16-чной системе счисления. Например, символ `@` кодируется в utf-8 кодом 64, или 40<sub>16</sub> в 16-чной системе счисления. Соответственно, последовательность `\x40` вставляет в строку байт с кодом 64, и `echo "\x40";` выведет символ `@`. С помощью таких кодов в строку мождно вставить любые байты, от `\x00` (код 0) до `\xff` (код 255). 
