...
- пробел (код 32)
- управляющие символы с кодами 0-31 и 127
...
- LF (код 10), line feed, перевод строки. Переставляет курсор на начало новой строки. В языках программирования PHP, Javascript и многих других обозначается последовательностью `\n`.
    ...
    В текстовых файлах обозначает конец строки (однако, под Windows по историческим причинам принято обозначать конец строки двумя идущими подряд символами, `CR LF` (коды 13 и 10)). 

- CR (код 13), carriage return, переставляет курсор в начало строки. Используется в текстовых файлах под Windows совместно с LF, чтобы обозначить конец строки. 
...
Поскольку один байт не позволяет вместить код символа из Юникода, для их хранения приходится выделять несколько байт. 2 байта содержат 16 бит, которыми вместе можно закодировать число от 0 до 65535. В первых версиях Юникода было меньше 65 000 символов, и поначалу их решили кодировать 2 байтами (UTF-16), позже, когда символов стало больше, символы решили кодировать 4 байтами (UTF-32, они позволяют закодировать чуть более 4 млрд. символов). Но даже с 2 байтами на символ разработчики умудрились сделать 2 (две!) разновидности их кодирования: UTF-16 LE и UTF-16 BE. Они отличаются друг от друга порядком байт. Если в одной версии кодировке символ кодируется байтами `A B`, то в другой - идущими в обратном порядке байтами `B A`. Аналогично и у UTF-32 есть 2 разновидности, где байты идут в прямом или обратном порядке. Код символа один и тот же, но он записывается в памяти разными способами. 

В UTF-16 символы с кодами менее 55296 (D800 в 16-чной системе счисления) кодируются 2 байтами, и символы с кодами выше (которые появились позже) кодируются как *суррогатная пара*, 4 байтами. 
...
[Подробнее про UTF-8](https://ru.wikipedia.org/wiki/UTF-8)
