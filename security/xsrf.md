...
```html
<!-- Указываем в качестве адреса отправки сайт банка и делаем форму невидимой, чтобы не пугать пользователя -->
...
Если (у пользователя нет куки с токеном) {
    Генерируем случайный код (например, 32 символа из диапазона a-zA-Z0-9) и сохраняем в куки пользователю;
} иначе {
...
Точно так же с помощью токена можно защитить и действия, выполняемые по ссылке (хотя по идее для таких вещей должны использоваться формы). Например, ссылка на форму логина с токеном может выглядеть как `/logout.php?token=abcdef123456` (именно так выглядят ссылки на многих сайтах).
