...
Можно проверить эти заголовки на соответствие домену сайта. Это не всегда надежно, так как пользователь мог отключить передачу `Referer` (в этом случае можно проверить `Origin`), а также, есть способы отправлять форму без него (способов подделать его вроде бы нету). В случае, если оба заголовка отсутствуют, нужно отказаться принимать данные формы (чтобы злоумышленник не мог обойти защиту, передав запрос без этих заголовков), но стоит логгировать такие факты, чтобы убедиться, что таких пользователей нету.
