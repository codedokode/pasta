...
Давай обсудим, как лучше решить эту задачу. Первое, с чего мы должны начать - это Модель. Вообще, модель в MVC может обозначать не один класс, а целую часть приложения, но тут она будет состоять ровно из одного класса, который можно назвать например MinesweeperGame или MinesweeperModel.  Внутри она должна хранить состояние игры (завершена, продолжается) и игрового поля (координаты мин, открытые клеточки). Тут есть 2 варианта, как это хранить: 
...
Также, в представлении могут быть методы установки обработчиков событий (напрмер "задать обработчик клика по клетке"). Это позволит контроллеру не работать с DOM напримую, а устанавливать обработчики событий через View.
...
- надо не забывать вписывать генерацию события при каждом изменении, иначе легко получить баг
...
В команде в свойствах хранятся параметры команды (например, координаты клеточки). Также в ней может храниться ссылка на модель. У команды есть 2 метода: `execute()` и `cancel()`. Первый сохраняет старое состояние модели (например состояние открываемой клеточки) и выполняет действия над ней, например просит ее открыть клеточку. Метод `cancel()` отменяет действие `execute()`, например просит модель вернуть клеточку в прежнее состояние.
...
```js
historyManager.execute(openCmd1);
historyManager.execute(openCmd2);
