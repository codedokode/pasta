...
~~В старом Zend Framework 1 был такой  класс, `Zend_Db_Table`, который содержал общие методы работы с таблицами и мог служить основой для TDG-классов: http://framework.zend.com/manual/1.12/ru/zend.db.table.html — правда, там по умолчанию результаты выборок возвращаются в виде похожих на массивы объектов класса `Zend_Db_Table_Row` (которые играли роль Row Data Gateway), а не в виде моделей твоего класса (но это можно переопределить опцией `rowClass`). ~~
