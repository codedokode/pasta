...
В этом варианте мы не используем никаких классов, а просто загружаем/вставляем данные в базу с использованием PDO. Для передчи данных мы используем обычные массивы:
...
Этот способ не требует создания никаких классов, но он очень ограничен и ведет к плохому коду, если в нашем приложении больше 1-2 таблиц: ты начнешь путаться, где какой массив и какие у него поля (например в данном случае, чтобы понять какие поля будут в `$news`, нам надо лезть в базу данных). В общем, в сложном приложении с таким подходом ничего хорошего нам не светит.
...
Это паттерн, когда мы для каждой таблицы в базе данных создаем отдельный класс (шлюз к данным таблицы = gateway, отсюда название паттерна) и создаем в этом классе методы работы с таблицей. Главное преимущство: SQL запросы больше не разбросаны по коду, а сгруппированы в отдельные классы. Если мы меняем что-то в таблице, нам не надо бегать по всему коду, нам достаточно взять работающий с этой таблицей класс и поправить только его.

Разумеется класс TDG не имеет права лезть в глобальные переменные вроде `$_GET`, `$_POST`, куки, и вообще делать что-то, кроме работы со своей таблицей в базе данных. Все нужные данные ему передаются через аргументы функций.
...
В Zend Framework есть такой  класс, `Zend_Db_Table`, который содержит общие методы работы с таблицами и может служить основой для TDG-классов: http://framework.zend.com/manual/1.12/ru/zend.db.table.html — правда, там по умолчанию результаты выборок возвращаются в виде похожих на массивы объектов класса `Zend_Db_Table_Row`, а не в виде моделей твоего класса (но это можно переопределить опцией `rowClass`). 

Описание: <http://design-pattern.ru/patterns/table-data-gateway.html>

Преимущества этого паттерна: простота реализации, изоляция SQL запросов работы с таблицей в одном классе, минусы: надо писать рутинные методы вроде получения объекта по id. 
...
Этот подход относительно прост, но он имеет недостаток: мы смешиваем код модели (методы работы со свойствами новости) и работу с таблицей новостей в одном классе. Хотя объект-новость вполе может сущестовать и сам по себе. Для решения этой проблемы нам нужен DataMapper.
...
Этот подход похож на паттерн Table Data Gateway. Но Data Mapper обычно устроен сложнее. Например в TDG, чтобы обновить заголовок новости, ты должен явно вызвать соответствуюший метод: 
...
Эти паттерны используются не сами по себе, а вместе с вышелописанными. 
