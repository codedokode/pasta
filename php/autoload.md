...
Эта строчка говорит о том, что далее в коде название `Router` будет обозначать класс `Symfony\Routing\Router` (иногда это еще называют "импортируем класс Router"). Если выбранный синоним совпадает с коротким названием класса, то конструкцию `as` можно не писать, и код получается еще проще:
