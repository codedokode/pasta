...
Оператор `??` (появился в PHP7) используется, чтобы при отсутствии ключа `age` в массиве `$_POST` не было ошибки. В PHP5 придется писать:

```php
$age = intval(array_key_exists('age', $_POST) ? $_POST['age'] : 0);
```
