...
Далее, введем функцию F(i). Эта функция определяет, какие суммы возможно выдать, используя только первые i купюр из списка. Запись F(2) = [0, 1000, 2000] обозначает, что, используя первые 2 купюры, мы можем выдать лишь 0, 1000 или 2000 рублей. «Выдать 0 рублей» звучит нелогично, но это нужно для работы алгоритма.

Значение функции F(0) = [0], то есть, используя ноль купюр, мы можем выдать лишь 0 рублей. Нетрудно посчитать значение функции при i = 1, 2, 3:
...
Перейдем к F(2), а после нее к F(3), добавив купюры a<sub>2</sub> и a<sub>3</sub>. Массив примет вид: 
...
Теперь перейдем к F(4), добавив купюру a<sub>4</sub> в 500 рублей. Для этого мы берем все существующие ключи из массива, добавляем к ним 500, и, если такого ключа нет, создаем его, записывая справа 500:
