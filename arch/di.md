...
- нельзя создать класс, не передав ему зависимости
...
[Registry (*реестр*)](http://design-pattern.ru/patterns/registry.html) - это класс, как правило, со статическими методами, который хранит в себе другие объекты. В начале скрипта мы помещаем объекты в `Registry`, и затем другие классы могут оттуда их брать. Например: 
...
- зависимости класса не видны явно. Непонятно, что надо положить в `Registry`, чтобы объект работал.
...
[ServiceLocator](http://sergeyteplyakov.blogspot.ru/2013/03/di-service-locator.html) это разновидность `Registry` с обычными, не статическими методами. Мы создаем `ServiceLocator`, заполняем его объектами, и передаем в конструктор, а класс может взять из него то, что ему требуется:
...
class UserDataGateway
{
    ...
    public function __contruct(ServiceLocator $sl)
    {
        $this->sl = $sl;
    }
...
Этот подход имеет такие недостатки: 
